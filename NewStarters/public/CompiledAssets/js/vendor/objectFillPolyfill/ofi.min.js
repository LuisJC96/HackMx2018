...
$(function () {
  var classesToCheck = ".fi-module-latest-videos__carousel, .fi-module-player__bg, .fi-module-focus, .fi-module-ranking__content__bg";

  $(classesToCheck).find('img').each(function (i, e) {
    ...
    if (fontFamily.indexOf("object-fit") != -1) {
      var objectFillProperties = fontFamily.split(";");
      for (i = 0; i < objectFillProperties.length; i++) {
        var prop = objectFillProperties[i].replace(/['"]+/g, '').split(":");
        $(this).attr("data-" + prop[0], prop[1]);
      };
    }
  });
});